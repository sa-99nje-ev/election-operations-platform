...
Apply the complete migration chain:alembic upgrade head

Check the current migration:alembic current

Display migration history:alembic history
...
Run the performance test suite: pytest tests/performance/ -v

Run coverage analysis: pytest --cov=app --cov-report=xml --cov-report=term-missing tests/
...
Run the critical Flake8 checks:  flake8 app/ tests/ --count --select=E9,F63,F7,F82 --show-source --statistics

Run the complete Flake8 check: flake8 app/ tests/

Check Git whitespace errors: git diff --check
